...
def main():
    ...
    try:
        ...
        # VAD 초기화 (라즈베리파이에서 더 나은 인식을 위해 파라미터 조정)
        # energy_threshold를 낮춰서 더 민감하게, min_speech_duration을 늘려서 더 긴 음성만 인식
        vad = EnergyBasedVAD(
            energy_threshold=0.005,  # 더 민감하게 (0.01 -> 0.005)
            silence_duration=0.8,  # 침묵 시간 약간 증가 (0.5 -> 0.8)
            min_speech_duration=0.3,  # 최소 음성 시간 증가 (0.15 -> 0.3)
        )
